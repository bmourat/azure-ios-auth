...
class ToDoTableViewController: UITableViewController, NSFetchedResultsControllerDelegate, ToDoItemDelegate {
  ...
  override func viewDidLoad() {
    ...
    let client = MSClient(applicationURLString: "https://vmubays.azurewebsites.net")
    let managedObjectContext = (UIApplication.shared.delegate as! AppDelegate).managedObjectContext!
    ...
    if let token = UserDefaults.standard.string(forKey: "token") {
      login(client: client, token: token, block: afterLogin)
    } else {
      firstLogin(client: client, block: afterLogin)
    }
  }

  func login(client: MSClient, token: String, block: @escaping () -> Void) {
    client.login(withProvider: "windowsazureactivedirectory", token: ["access_token" : token]) { (user, error) in
      if error == nil {
        block()
      } else {
        print("Error during login: \(String(describing: error))")
      }
    }
  }

  func firstLogin(client: MSClient, block: @escaping () -> Void) {
    let appDelegate = UIApplication.shared.delegate as! AppDelegate
    appDelegate.todoTableViewController = self
    client.login(withProvider: "windowsazureactivedirectory", urlScheme: "vmubays", controller: self, animated: true) { (user, error) in
      if let user = user {
